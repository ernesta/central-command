...
export async function rewriteNoteFiles({
  ...
  transform,
  backupDir
}: RewriteOptions): Promise<RewriteReport> {
  let names: string[]
  try {
    names = (await readdir(dir)).filter((f) => f.endsWith('.md') && !f.startsWith('.')).sort()
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') return { changed: 0, skipped: [] }
    throw error
  }
  const report: RewriteReport = { changed: 0, skipped: [] }
